...
def get_quote_of_the_day():
    ...
    test = 1
    url = "https://api.quotery.com/wp-json/quotery/v1/quotes?orderby=popular&page="+str(page_number)+"&per_page=120"
				...
				if index > 0:
					for inner_index,quote in enumerate(quote_element):
						if inner_index == 0:
							...
							cleaned_quote = quote[2:len(quote)-2].encode('utf-8').decode('unicode-escape')
				...
				if index > 0:
					for inner_index,author in enumerate(author_element):
						if inner_index == 0:
							...
							cleaned_author = author[2:len(author)-2].encode('utf-8').decode('unicode-escape')
				...
				for topic in expected_topic:
					...
					while test:
						...
						for author in expected_author:
							quote, authors, test = scraper(url,test)
							quotes_and_authors += selection_author(quote,authors,author)
						return quotes_and_authors
			else:
...
# print(get_quote("Martin Luther King Jr.","Black History"))