...
def get_quotes(person: (None, str)=None, category: (None,str)=None):
	...
	if not isinstance(person,type(None)):	#person's name is provided
		Person = "-".join(person.strip().split())	#formatting the name to fit as per URL
	...
	if not isinstance(category,type(None)):	#category's name is provided
		category_link = requests.get(parent_link + "/quotes/categories/" + category).text
...
def quote_of_the_day():
	...
	quote = soup_obj.find('div',class_='clearfix').find('a',title='view quote').text
	author = soup_obj.find('div',class_='clearfix').find('a',title='view author').text
	return (quote,author)
